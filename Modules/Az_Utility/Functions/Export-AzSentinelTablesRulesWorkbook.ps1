...
Function Export-AzSentinelAnalyticsRules {
    param (
        ...
        [Parameter(Mandatory = $false)] [string]$FileName = "Sentinel_CurrentRules_testing.csv" #default
    )
    ...
    #First API call to grab all of the existing tables for the client
    $url = "https://management.azure.com/subscriptions/$($subscriptionId)/resourceGroups/$($resourceGroupName)/providers/Microsoft.OperationalInsights/workspaces/$($workspaceName)/providers/Microsoft.SecurityInsights/metadata?api-version=2017-01-01-preview"
    ...
    #Adding custom table/function from the client
    $tables.Add('Sysmon')
    ...
    foreach ($result in $results) {
        ...
        if( $query -ne ""){
            ...
            foreach($table2 in $tables) {
                if($query.Indexof($table2)) {
