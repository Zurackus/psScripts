        ...
        if($connection -ne $null)
        {
            ...
            if($connectorDictionary.ContainsKey($connectorIdLower))
            {
                ...
                $matchingConnector.Playbook += "$logicAppName, "
                    $logicAppJsonText.properties.changedTime
                    $logicAppJsonText.properties.createdTime
                    $logicAppJsonText.properties.state
...
$csvFilePath = Join-Path -Path $pwd -ChildPath "OrphanedConnectors2.csv"
